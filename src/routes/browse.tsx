...
export const Route = createFileRoute("/browse")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Explore all Mozok courses across design, code, languages and security.",
      },
    ...
    <div className="min-h-screen bg-background" style={{ fontFamily: "Poppins, sans-serif" }}>
      <div className="mx-auto max-w-7xl px-6 py-10">
        <div className="flex items-center justify-between">
          ...
          <Link
            to="/auth"
            className="rounded-full bg-black px-5 py-2 text-sm font-semibold text-background"
          >
            Sign in
          </Link>
        </div>
        <div className="relative mt-10 overflow-hidden rounded-3xl border border-border bg-card px-8 py-10">
          <img
            ...
            aria-hidden
            loading="lazy"
            className="pointer-events-none absolute -right-6 -top-6 h-40 w-40 opacity-90"
          />
          <img
            src={doodleRocket}
            alt=""
            aria-hidden
            loading="lazy"
            className="pointer-events-none absolute -bottom-8 right-32 hidden h-32 w-32 opacity-80 md:block"
          />
          <h1 className="text-4xl font-bold md:text-5xl">Browse all courses</h1>
          <p className="mt-2 text-muted-foreground">
            Find your next skill. {courses.length} courses available.
          </p>
        </div>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {cats.map((c) => (
            <button
              key={c}
              onClick={() => setCat(c)}
              className={`rounded-full px-4 py-1.5 text-sm font-medium transition ${cat === c ? "bg-black text-background" : "bg-card text-foreground hover:bg-secondary"}`}
            >
              {c}
            </button>
          ))}
...
function BrowseCard({ course }: { course: CourseCard }) {
  return (
    <Link
      ...
      className="group block rounded-3xl bg-card p-6 ring-1 ring-border transition-all hover:-translate-y-1 hover:"
    >
      ...
      <div className="mt-6 flex items-center justify-between">
        <div className="rounded-full bg-secondary px-3 py-1.5 text-xs font-semibold">
        ...
        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-foreground text-background transition-transform group-hover:translate-x-1">
