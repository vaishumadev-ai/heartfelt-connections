    ...
    <div className="min-h-screen bg-[#f5f5f5]" style={{ fontFamily: "'Poppins', sans-serif" }}>
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        <div className="mt-6 flex items-center rounded-full bg-card p-1.5">
          ...
          <button className="flex h-11 w-11 items-center justify-center rounded-full bg-[#ff5a6a] text-background"><Search className="h-5 w-5" /></button>
        </div>
  ...
  return (
    <Link to="/courses/$slug" params={{ slug: course.slug }} className="group block rounded-3xl bg-card p-6 ring-1 ring-border transition-all hover:-translate-y-1 hover:">
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#ff5a6a] text-background transition-transform group-hover:translate-x-1"><ArrowRight className="h-4 w-4" /></div>
      </div>
    </Link>
  );
}