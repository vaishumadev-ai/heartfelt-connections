        ...
        <div className="mt-6 grid grid-cols-1 gap-8 lg:grid-cols-[1fr_360px]">
          <div className="rounded-3xl bg-white p-8 shadow-[0_10px_40px_-20px_rgba(0,0,0,0.15)]">
            ...
            <div className="mt-6 flex items-center gap-6 text-sm text-gray-500">
              <span className="flex items-center gap-1"><Heart className="h-4 w-4 fill-[#ff5a6a] text-[#ff5a6a]" /> {course.likes}</span>
            ...
            <ul className="mt-4 space-y-2">
              {course.lessons.length === 0 && <li className="text-sm text-gray-500">Lessons coming soon.</li>}
              {course.lessons.map((l, i) => (
                <li key={l.id} className="flex items-center gap-4 rounded-2xl bg-[#f9f9f9] p-4">
                  <div className="flex h-9 w-9 items-center justify-center rounded-full bg-white text-sm font-bold text-gray-600">{i + 1}</div>
                  <div className="flex-1">
                    <div className="font-semibold">{l.title}</div>
                    {l.duration_seconds && <div className="text-xs text-gray-500">{Math.round(l.duration_seconds / 60)} min</div>}
                  </div>
          ...
          <aside className="space-y-4">
            <div className="rounded-3xl bg-white p-6 shadow-[0_10px_40px_-20px_rgba(0,0,0,0.15)]">
              ...
              {userId ? (
                ...
                <Link to="/auth" className="mt-4 flex w-full items-center justify-center rounded-full bg-black px-6 py-3 text-sm font-semibold text-white">Sign in to enroll</Link>
              )}
              <ul className="mt-6 space-y-2 text-sm text-gray-600">
                <li className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-[#4aa9c9]" /> Full lifetime access</li>
                <li className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-[#4aa9c9]" /> Certificate of completion</li>
                <li className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-[#4aa9c9]" /> Learn at your own pace</li>
              </ul>
            </div>
          </aside>
        </div>
